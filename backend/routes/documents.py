...
def analyze_screening(
    ...
    try:
        with ANALYSIS_LOCK:
            ...
            _t_ocr_end = _time.time()
            print(f"[OCR] OCR COMPLETED ({len(ocr_candidate_text)} chars) [{_t_ocr_end - _t_ocr_start:.1f}s]")

            # 2. Execute Gemini multimodal vision analysis
            _t_gem_start = _time.time()
            try:
            ...
            _t_face_end = _time.time()
            print(f"[FACE] FACE ANALYSIS COMPLETED [{_t_face_end - _t_face_start:.1f}s]")
            ...
            # Uses CORROBORATING EVIDENCE — a single weak signal does not cause FAKE
